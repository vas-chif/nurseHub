...
1. **Creazione:** L'utente sceglie data e turno da offrire.
   - _Logic Check:_ Deve avere quel turno in quella data (Validazione client-side).
...
4. **Approvazione:** L'admin conferma.
   ...
   - **Google Sheets Sync**: Se in modalità "Auto", viene aggiornato il file Master tramite `api/update-sheet-swap.js`.

- **Replicabilità 100% (Standalone Module)**: Tutta la logica di sincronizzazione risiede nel file standalone `Sincronizzazione_NurseHub.gs`. Questo modulo può essere aggiunto a qualsiasi foglio Google esistente senza modificare gli script originali del proprietario (`Codice.gs`, `macros.gs`), garantendo un'integrazione non invasiva e portabile.
...
- **Sorting & Filters Persistenti**: Lo stato dei filtri (operatori, date, codici turno, note) e l'ordinamento vengono salvati automaticamente nel `localStorage`.
- **Filtro Temporale Unificato**: Introduzione del "Periodo" come filtro globale. Admin può impostare un intervallo date (Dal/Al) o usare scorciatoie rapide (Oggi, +7gg, Mese Corrente) per filtrare simultaneamente Richieste di Assenza, Cambi Turno e Notifiche storiche.
