/** * @file SystemConfig.vue * @description Advanced administrative panel for managing hospital
configurations and scenarios. * @author Nurse Hub Team * @created 2026-03-24 * @modified 2026-04-23
*/
...
const groupedConfigurations = computed(() => {
  const groups: Record<string, SystemConfiguration[]> = {};
  configurations.value.forEach((config) => {
    const g = config.group || 'Altre Configurazioni';
    if (!groups[g]) groups[g] = [];
    groups[g].push(config);
  });
  return Object.entries(groups).sort(([a], [b]) => a.localeCompare(b));
});

const newConfigGroup = ref('');
...
function openRenameDialog(name: string) {
  if (name === 'Altre Configurazioni') return;
...
async function renameGroup() {
  if (!newGroupName.value || newGroupName.value === oldGroupName.value) {
  ...
  try {
    ...
    await batch.commit();
    $q.notify({ type: 'positive', message: 'Reparto rinominato con successo!' });
    await configStore.loadConfigurations();
    showRenameDialog.value = false;
  } catch (error) {
    logger.error('Rename Group Error', error);
    $q.notify({ type: 'negative', message: 'Errore durante la rinomina' });
  } finally {
...
<template>
  <q-card>
    ...
    <q-list separator bordered class="q-py-xs">
      ...
      <div v-for="[groupName, groupConfigs] in groupedConfigurations" :key="groupName">
        <q-item-label
          header
          class="text-uppercase text-weight-bolder text-primary bg-blue-1 q-py-sm row items-center justify-between"
        >
          <div class="row items-center">
            <q-icon name="folder" class="q-mr-xs" /> {{ groupName }}
          </div>
          <q-btn
            v-if="groupName !== 'Altre Configurazioni'"
            flat
            round
            dense
            icon="edit"
            size="sm"
            color="primary"
            @click="openRenameDialog(groupName)"
          >
            <q-tooltip>Rinomina Reparto</q-tooltip>
          </q-btn>
        </q-item-label>
        ...
        >
          <template v-slot:header>
            <q-item-section avatar>
              <q-avatar
                ...
                :color="config.id === configStore.activeConfigId ? 'primary' : 'grey-5'"
                text-color="white"
              />
            </q-item-section>
            <q-item-section>
          ...
          <q-expansion-item
            ...
            default-opened
            header-class="text-subtitle1 text-weight-bold bg-grey-2"
            class="q-ma-md q-mb-md bg-white rounded-borders shadow-2 overflow-hidden"
            style="border: 1px solid #ddd"
          >
            <q-card flat>
              <q-card-section class="q-gutter-y-md q-pa-lg">
                <!-- Row 1: Name, Profession, Group -->
                ...
                >
                  <template v-slot:append>
                    <q-btn
                      ...
                      icon="assignment"
                      color="grey-7"
                      @click="pasteUrl(config, 'spreadsheet')"
                    >
                      <q-tooltip>Incolla dagli appunti</q-tooltip>
                    </q-btn>
                  </template>
                </q-input>

                <!-- Row 3: GAS Web App URL -->
                <q-input
              ...
              <q-card-actions class="q-px-lg q-py-md row items-center">
                ...
                <div
                  v-if="config.id === configStore.activeConfigId"
                  class="row items-center text-green q-mr-auto"
                >
                  <q-icon name="check_circle" size="sm" class="q-mr-xs" />
                  <span class="text-subtitle2 text-weight-bold">Configurazione Attiva</span>
                </div>
                <q-btn
                  v-else
                  flat
                  label="Attiva Configurazione"
                  color="primary"
                ...
                <div class="row q-gutter-sm">
                  ...
                  <q-btn
                    ...
                    :loading="saving && savingId === config.id"
              ...
              <q-card-section>
                <div class="row items-center justify-between q-mb-md">
                  <div class="text-caption text-grey">
                    Regole per la copertura dei turni scoperti
                  </div>
                ...
                <div v-else-if="scenariosLoaded[config.id]">
                  <div
                    v-if="
                      !scenariosByConfig[config.id] || scenariosByConfig[config.id]!.length === 0
                    "
                            ...
                            <div class="col">
                              ...
                              <div class="row q-gutter-xs q-mt-xs">
                                ...
                                >
                                  ...
                                  <span
                                    v-if="idx < scenario.roles.length - 1"
  ...
  <q-dialog v-model="showCreateDialog">
    <q-card style="min-width: 400px">
      ...
      <q-card-section class="q-gutter-md q-pt-lg">
        ...
        />
        <q-select
          v-model="newConfigGroup"
          :options="uniqueGroups"
          label="Gruppo / Area (Opzionale)"
          outlined
          use-input
          fill-input
          hide-selected
  ...
  <q-dialog v-model="showRenameDialog" persistent>
    <q-card style="min-width: 400px">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Rinomina Reparto</div>
        <q-space />
        <q-btn icon="close" flat round dense v-close-popup :disable="renaming" />
      </q-card-section>
      ...
      <q-card-actions align="right" class="q-pa-md">
        <q-btn flat label="Annulla" v-close-popup :disable="renaming" />
        <q-btn
