...
<template>
  ...
  <q-card v-if="step === 'form'" flat bordered class="absence-form-card q-mb-md shadow-1">
    ...
    <q-card-section class="q-gutter-md">
      ...
      <q-select v-if="isAnyAdmin" v-model="selectedOperatorId" :options="filteredOperatorOptions" option-label="name"
      ...
      <div class="row q-col-gutter-md items-start">
        ...
        <div class="col-12 col-md-4">
          <AppDateInput v-model="formData.date" label="Data Assenza" required
            :hint="formData.isRecurring ? 'Data inizio' : ''" />
        </div>

        <!-- Recurring Toggle -->
        <div class="col-12 col-md-4 flex items-center">
          <q-toggle v-model="formData.isRecurring" label="Più giorni" color="secondary" dense />
        </div>
      ...
      <!-- Absence Type -->
      <q-select v-model="formData.absenceLabel" :options="absenceOptions" label="Motivo Assenza" outlined dense
        emit-value map-options />

      <!-- Note -->
      <q-input v-model="formData.note" label="Note (opzionale)" outlined dense type="textarea" rows="2" />
    </q-card-section>

    <q-card-actions align="right" class="q-pb-md q-pr-md">
      <q-btn label="Genera Anteprima" color="primary" unelevated icon-right="preview" :loading="generatingPreview"
  ...
  <q-card v-else flat bordered class="absence-form-card q-mb-md shadow-1">
    ...
    <q-card-section v-else class="q-pa-none">
      <q-list bordered separator>
        <q-item v-for="row in previewRows" :key="row.date" :class="{
          ...
          <q-item-section style="min-width: 110px; max-width: 130px">
            <q-item-label class="text-weight-bold text-body2">
              ...
              <q-icon v-if="row.conflict && !row.excluded" name="warning" color="warning" size="xs" class="q-ml-xs">
          ...
          <q-item-section class="q-px-xs">
            <q-select v-model="row.selectedShift" :options="shiftOptions" emit-value map-options dense outlined
              label="Turno" :disable="row.excluded" />
          </q-item-section>

          <!-- Reason per row -->
          <q-item-section class="q-px-xs">
            <q-select v-model="row.absenceLabel" :options="absenceOptions" emit-value map-options dense outlined
              label="Motivo" :disable="row.excluded" />
          </q-item-section>

          <!-- Note per row (visible and editable for operator timbratura instructions) -->
          <q-item-section class="q-px-xs">
            <q-input v-model="row.note" dense outlined label="Note" :disable="row.excluded" clearable />
          </q-item-section>

          <!-- Exclude toggle -->
          <q-item-section side>
            <q-toggle v-model="row.excluded" color="negative" size="sm">
    ...
    <q-card-actions align="right" class="q-pb-md q-pr-md q-pt-md">
      ...
      <q-btn :label="`Conferma e Invia (${previewActiveCount})`" color="positive" unelevated icon="send"
        :loading="submitting" :disable="previewActiveCount === 0" @click="confirmAndSubmit" />
    </q-card-actions>
  </q-card>
</template>
