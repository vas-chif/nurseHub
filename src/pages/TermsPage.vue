/**
 * @file TermsPage.vue
 * @description Page for viewing Terms and Conditions of the Nurse Hub platform.
 * @author Nurse Hub Team
 * @created 2026-03-10
 * @modified 2026-04-27
 * @notes
 * - Uses a drawer-based navigation for different legal sections.
 * - Content is dynamically loaded from terms.ts data.
 */
<script setup lang="ts">
import { ref } from 'vue';
import { useRouter } from 'vue-router';
import { termsIntro, btnTabTerms, tabsMenuTermsGeneral, tabsMenuTermsUse } from '../data/terms';
...
// Initialize
const initialSection = tabsMenuTermsGeneral.value.find((item) => item.clickable && item.dettagli);
if (initialSection) {
  goTo(initialSection);
}
</script>
    ...
    <q-layout view="hHh lpR fFf" class="rounded-borders">
        ...
        <q-page class="bg-grey-2 q-pa-md">
          ...
          <q-expansion-item default-opened class="bg-white shadow-1 rounded-borders q-mb-md">
            <template v-slot:header>
              ...
              <q-item-section>
                <q-item-label class="text-h6">{{ termsIntro[0]?.title }}</q-item-label>
                <q-item-label caption>{{ termsIntro[1]?.subtitle }}</q-item-label>
              </q-item-section>
            </template>

            <q-card>
              <q-card-section>
                <div class="text-body1">{{ termsIntro[2]?.paragrafo }}</div>
              </q-card-section>
            </q-card>
          </q-expansion-item>
      ...
      <q-drawer v-model="leftDrawerOpen" show-if-above :mini="miniState" @click.capture="drawerClick" :width="280"
        ...
        <q-scroll-area class="fit">
          <q-list padding>
            <!-- Back button -->
            <q-item v-for="(item, index) in btnTabTerms" :key="'btn-' + index" clickable v-ripple
            ...
            <!-- Sections -->
            <template v-for="(section, sIndex) in [tabsMenuTermsGeneral, tabsMenuTermsUse]" :key="sIndex">
              ...
              <q-separator class="q-my-sm" v-if="sIndex < 1" />
            </template>
            ...
            <q-item clickable v-ripple to="/license">
              ...
              <q-item-section>
                <q-item-label>Licenze Software</q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        </q-scroll-area>
