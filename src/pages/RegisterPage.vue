 ...
 * @modified 2026-04-27
 * @notes
 * - Collects PII (name, email, DOB) and credentials.
 * - Forces acceptance of Terms and Conditions.
 * - Triggers Firebase email verification flow upon successful registration.
 */
    ...
    <q-card class="q-pa-md" style="width: 400px; max-width: 90vw">
      ...
      <q-card-section>
        <q-form @submit="handleRegister" class="q-gutter-md">
          <q-input v-model="formData.firstName" filled label="Nome" :rules="[(val) => !!val || 'Nome obbligatorio']" />

          <q-input v-model="formData.lastName" filled label="Cognome"
            :rules="[(val) => !!val || 'Cognome obbligatorio']" />

          <q-input v-model="formData.email" filled type="email" label="Email" autocomplete="email"
            :rules="[(val) => !!val || 'Email obbligatoria']" />

          <q-input v-model="formData.dateOfBirth" filled label="Data di Nascita" mask="####-##-##"
          ...
          <q-input v-model="formData.confirmPassword" filled :type="isPwdConfirm ? 'password' : 'text'"
          ...
          <q-btn type="submit" label="Registrati" color="primary" :loading="loading" class="full-width"
            :disable="!acceptTerms" />

          <div class="text-center q-mt-md">
            <q-btn flat label="Hai già un account? Accedi" color="primary" @click="$router.push('/login')" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
    ...
    <q-dialog v-model="showVerificationDialog" persistent transition-show="scale" transition-hide="scale">
      <q-card class="bg-primary text-white" style="width: 300px">
        ...
        <q-card-section class="q-pt-none">
          Un'email di verifica è stata inviata a <strong>{{ formData.email }}</strong>. <br /><br />
