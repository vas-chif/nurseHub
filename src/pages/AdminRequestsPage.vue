 ...
 * @description Admin page for managing absence/shift requests and swap proposals.
 *   Orchestrates useAdminRequests, useAdminSwaps, and smart-suggest logic.
 *   Provides reactive context to sub-components via Vue provide/inject.
 * @author Nurse Hub Team
 * @created 2026-03-22
 * @modified 2026-05-05
 * @notes
 * - §1.11 compliant: logic extracted to composables; template panels to sub-components.
 * - Sub-components inject 'adminReqsContext', 'adminSwapsContext', 'adminSmartContext'.
 */
...
function calculatePriority(op: Operator, currentShift: ShiftCode, targetShift: ShiftCode, date: string): number {
    ...
    .forEach((scen) => {
      const group: ScenarioGroup = {
        ...
        positions: scen.roles.map((role) => ({
          ...
          ...(role.requiredNextShift !== undefined ? { requiredNextShift: role.requiredNextShift } : {}),
          candidates: [],
        })),
      };
      ...
      group.positions.forEach((pos) => {
        Object.values(reqs.operators.value).forEach((op) => {
          ...
          const nextDate = new Date(new Date(req.date).getTime() + 86400000).toISOString().split('T')[0]!;
...
function getSuggestions(id: string): ScenarioGroup[] { return suggestions.value[id] ?? []; }
...
async function publishRequest(req: ShiftRequest): Promise<void> {
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: `Richiesta pubblicata a ${selected.length} operatori!` });
  } catch (e) {
...
watch(() => configStore.activeConfigId, async (v) => {
  if (v) {
    await reqs.fetchOperators();
...
watch(() => scheduleStore.lastUpdated, () => void reqs.fetchOperators());
        ...
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-md-3">
            <q-select
              ...
              multiple use-chips dense outlined emit-value map-options clearable
            />
          </div>
          <div class="col-12 col-md-3">
            <q-select
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-section>
          <q-input
            ...
            type="textarea" label="Inserisci il motivo" rows="4" outlined autofocus
            :rules="[(val) => !!val || 'Motivo obbligatorio']"
          />
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Annulla" v-close-popup />
          <q-btn flat label="Conferma Rifiuto" color="negative" @click="reqs.confirmReject()" :disable="!reqs.rejectionReason.value" />
        </q-card-actions>
      </q-card>
    </q-dialog>

    <!-- Approval Dialog -->
    <q-dialog v-model="reqs.showApprovalDialog.value" persistent>
      <q-card style="min-width: 350px">
        ...
        <q-card-section class="q-pt-none">
          <div v-if="reqs.approvalContext.value?.offer" class="q-mb-md">
            Stai accettando l'offerta di <strong>{{ reqs.approvalContext.value.offer.operatorName }}</strong>.
          </div>
          <div v-else-if="reqs.approvalContext.value?.req" class="q-mb-md">
            Stai confermando la copertura per
            <strong>{{ reqs.getOperatorName(reqs.approvalContext.value.req.absentOperatorId, reqs.approvalContext.value.req) }}</strong>.
          </div>
          <div class="bg-grey-2 q-pa-md rounded-borders">
            <div class="text-subtitle2 q-mb-sm">Sincronizzazione Google Sheets</div>
            <q-btn-toggle
              v-model="reqs.syncMode.value" spread no-caps rounded unelevated
              toggle-color="primary" color="white" text-color="primary"
              :options="[{ label: 'Automatica', value: 'auto' }, { label: 'Manuale', value: 'manual' }]"
            />
            <div class="text-caption text-grey-7 q-mt-sm">
              <span v-if="reqs.syncMode.value === 'auto'">Il turno verrà aggiornato automaticamente sul file Excel Master.</span>
        ...
        <q-card-actions align="right" class="text-primary">
          <q-btn flat label="Annulla" v-close-popup />
          <q-btn unelevated color="positive" label="Conferma & Chiudi" @click="reqs.processApproval()" :loading="reqs.loading.value" />
        </q-card-actions>
      </q-card>
    </q-dialog>

    <!-- Swap Approval Dialog -->
    <q-dialog v-model="swaps.showSwapApprovalDialog.value" persistent>
      <q-card style="min-width: 350px">
        <q-card-section class="row items-center">
        ...
        <q-card-section class="q-pt-none">
          <div v-if="swaps.approvalSwapContext.value" class="q-mb-md">
            Stai confermando il cambio turno del
            <strong>{{ reqs.formatDate(swaps.approvalSwapContext.value.date) }}</strong><br />
            tra
            <strong>{{ swaps.approvalSwapContext.value.creatorName || swaps.approvalSwapContext.value.creatorId }}</strong>
            e
            <strong>{{ swaps.approvalSwapContext.value.counterpartName || swaps.approvalSwapContext.value.counterpartId || 'Assegnazione' }}</strong>.
          </div>
          <div class="bg-grey-2 q-pa-md rounded-borders">
            <div class="text-subtitle2 q-mb-sm">Sincronizzazione Google Sheets</div>
            <q-btn-toggle
              v-model="swaps.swapSyncMode.value" spread no-caps rounded unelevated
              toggle-color="primary" color="white" text-color="primary"
              :options="[{ label: 'Automatica', value: 'auto' }, { label: 'Manuale', value: 'manual' }]"
            />
            <div class="text-caption text-grey-7 q-mt-sm">
              <span v-if="swaps.swapSyncMode.value === 'auto'">I turni invertiti verranno aggiornati automaticamente sul file Excel Master per entrambi gli operatori.</span>
              <span v-else>Dovrai aggiornare i turni degli operatori sul file Excel Master manualmente in un secondo momento.</span>
            </div>
          </div>
        </q-card-section>
        <q-card-actions align="right" class="text-primary">
          <q-btn flat label="Annulla" v-close-popup />
          <q-btn unelevated color="positive" label="Approva & Applica" @click="swaps.processSwapApproval()" :loading="swaps.swapLoading.value" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
