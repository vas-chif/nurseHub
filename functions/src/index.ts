...
async function sendNotificationToUser(
  identifier: { userId?: string; operatorId?: string },
  title: string,
  body: string,
  url: string = '/',
) {
  ...
  try {
    ...
    console.log(
      `Notifiche inviate a ${targetUserId}: ${response.successCount} successi, ${response.failureCount} fallimenti.`,
    );
...
export const onShiftRequestStatusChange = functions
  .region('europe-west1')
        ...
        for (const offer of newData.offers) {
          ...
          if (!offer.isRejected) {
            await sendNotificationToUser(
              { operatorId: offeringUserId },
              'Offerta Approvata! 🎉',
              `La tua disponibilità per il turno del ${newData.date} è stata accettata. Grazie!`,
              '/',
            );
          } else {
            ...
            await sendNotificationToUser(
              ...
              `La tua offerta per il turno del ${newData.date} non è stata confermata da parte dell'Admin.`,
              '/',
            );
          }
        }
...
export const onNewShiftRequest = functions
  .region('europe-west1')
  .firestore.document('shiftRequests/{requestId}')
